...
ALGORITHMS: Dict[str, Type[Any]] = {'DQN': DQN, 'PPO': PPO, 'SAC': SAC, 'TD3': TD3}
...
def main(args: Namespace) -> None:
    ...
    Raises:
        AssertionError: If time limit exceeds 1440 minutes or if number of environments is less than 1.
    ...
    run_name = args.resume_run_name if args.resume_run_name \
        else f"{args.alg}_{args.env_id}_{"T" if args.action_type == "topology" else "R"}_{args.seed}_{args.difficulty}_{"H" if args.use_heuristic else ""}_{"I" if args.heuristic_type == "idle" else ""}_{int(time())}_{np.random.randint(0, 50000)}"
    ...
    envs = gym.vector.AsyncVectorEnv([make_env(args, i, resume_run=checkpoint.resumed) for i in range(args.n_envs)])
